/**
* @Author wwy
* @Description  1.强引用：当内存空 间不足，Java虚拟机宁愿抛出OutOfMemoryError错误，
 *              使程序异常终止，也不会靠随意回收具有强引用的对象来解决内存不足问题。
 *              2.软引用：如果内存空间足够，垃圾回收器就不会回收它，如果内存空间不足了，
 *              就会回收这些对象的内存。只要垃圾回收器没有回收它，该对象就可以被程序使用。
 *              软引用可用来实现内存敏感的高速缓存。
 *              3.弱引用：只具有弱引用的对象拥有更短暂的生命周期。在垃圾回收器线程扫描它
 *              所管辖的内存区域的过程中，一旦发现了只具有弱引用的对象，不管当前内存空间足够与否，
 ...
 *              4.  "虚引用"顾名思义，就是形同虚设，与其他几种引用都不同，虚引用并不会决定对象的生命周期。
